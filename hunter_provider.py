...
class HunterProvider:
    """Lead provider that searches Google Maps via the Hunter agent.

    Strategy: "hunter" — Uses `buscar_leads_google_maps` from agente1_hunter_v2
    to scrape real leads from Google Maps for each (segmento, cidade) pair.
    """
    ...
    # ---- public API (called by lead_providers facade) ----
    ...
    async def search(
        ...
        *,
        ...
        """Run Hunter search for the given segmento/cidade pairs.

        Returns a list of raw lead dicts captured from Google Maps.
        """
        ...
        for segmento, cidade in pairs:
            ...
            try:
                ...
                self._log(
                    "error",
                    f"Hunter falhou em {segmento}/{cidade}: {str(exc)[:180]}",
                )
                continue
    ...
    def store_candidates(
        ...
        """Store each candidate into lead_inventory.
    ...
    def _store_candidate(
        ...
        row = self.db.execute(
            ...
            ),
            {
                ...
                # F1-truthful: use scraper's total_avaliacoes as-is (even when 0).
                # Previous `or len(reviews)` fabricated a non-zero count whenever
                # the scraper reported 0 but the reviews list was non-empty.
                "reviews_count": raw.get("total_avaliacoes")
                if raw.get("total_avaliacoes") is not None
                else len(raw.get("reviews") or []),
    ...
    def _log(self, level: str, message: str, payload: dict[str, Any] | None = None) -> None:
        ...
        _event(self.db, self.tenant_id, "hunter", level, message, payload)
