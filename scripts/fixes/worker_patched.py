"""Worker daemon â€” consome fila Postgres e dispara agentes.

Loop: claim_next() â†’ run_pipeline() â†’ mark done/failed.
"""
...
def _shutdown(signum, frame):
    global _running
    logger.info("Sinal %s recebido, parando apÃ³s prÃ³ximo job", signum)
    _running = False
...
def _load_job_tipos() -> tuple:
    """LÃª WORKER_JOB_TYPES do env (comma-separated). Fallback para hardcoded."""
...
def _run_pipeline_job(db, job) -> bool:
    ...
    job_id = job["id"]
    try:
        """pipeline_lead: roda o Manager e fecha o loop de inventÃ¡rio."""
        ...
        # busca os dados do lead no banco para que step_hunter nÃ£o falhe com
        # "Hunter sem lead" quando o job Ã© reprocessado sem payload completo.
        ...
        # â”€â”€ Observability: criar trace para este job â”€â”€
        trace = Trace(
            run_id=state.run_id,
        ...
        if success:
            ...
            try:
                ...
                log_pipeline_error(
                    ...
                    traceback_str=None,  # traceback jÃ¡ vai no log
                )
        ...
        # Loop-closer: atualiza lead_inventory e re-arma o prÃ³ximo tick.
        ...
        # â”€â”€ Observability: salvar trace â”€â”€
        try:
            salvar_trace(trace)
