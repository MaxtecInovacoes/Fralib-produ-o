...
DIRECOES_VISUAIS = {
    ...
    "flat": {"nome": "Flat", "tokens": {"--bg": "oklch(98% 0.003 250)", "--surface": "oklch(100% 0.0 0)", "--fg": "oklch(9% 0.017 221)", "--muted": "oklch(60% 0.01 0)", "--border": "oklch(90% 0.006 250)", "--accent": "oklch(51% 0.143 14)"}, "font_heading": "Inter", "font_body": "Inter", "vibe": "2D minimalista, cores vibrantes, sem efeitos 3D", "animation": "elegante"},
...
def get_design_context(segmento: str, nome_negocio: str = "", tier: str = "STANDARD", dark_mode: bool = False, od_slug: str = "", dados_lead: dict = None) -> dict:
    ...
    if _sub_nicho_data and _sub_nicho_data.get("sub_nicho") == "natacao":
        _palette_hue = 200  # Tom de piscina refrescante (azul/ciano)
    ...
    if _bg_l is not None:
        ...
        if _muted_l is not None:
            if is_light_theme and (_muted_l > 80 or _muted_l < 30):
                tokens["--muted"] = f"oklch(55% 0.015 {_palette_hue})"
            elif not is_light_theme and (_muted_l < 40 or _muted_l > 90):
                tokens["--muted"] = f"oklch(65% 0.015 {_palette_hue})"
        ...
        if _accent_needs_fix:
            tokens["--accent"] = f"oklch(55% 0.2 {_palette_hue})"
        ...
        if seg in _SEGMENT_HUE_STRICT and not _accent_needs_fix:
            ...
            if _current_hue is not None:
                ...
                _hue_diff = abs(_current_hue - _palette_hue)
                if _hue_diff > 180:
