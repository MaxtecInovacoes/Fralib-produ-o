...
def call_claude(
    ...
    if _ia and not base_url:
        try:
            # FASE 1 BYPASS_UNCONDITIONAL (FRA-LIB 2026-08-17): budget limit disabled permanently
            print("[LLM] Budget check BYPASSED (FRA-LIB 2026-08-17 — budget limit disabled)")
            ...
            # FASE 2 — Budget diario global (BYPASS_PERMANENTE FRA-LIB 2026-08-17)
            _budget_ok, _budget_remaining = True, float("inf")
            if not _budget_ok:
            ...
            # Fase 3 — Budget por tenant (BYPASS_PERMANENTE FRA-LIB 2026-08-17)
            _tenant_ok, _tenant_remaining = True, float("inf")
            if not _tenant_ok:
                print(
...
def call_claude_stream(
    ...
    if _db_config:
        ...
        if _db_config.get("temperature") is not None:
            temperature = float(_db_config["temperature"])
        _raw_max = _db_config.get("max_tokens")
        max_tokens = int(_raw_max) if _raw_max and str(_raw_max).strip().isdigit() else max_tokens
    else:
        model_id = current_map.get(model, current_map["opus"])
