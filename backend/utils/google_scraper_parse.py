"""
Google Local Scraper - Parse Module
Funções de parsing e extração de dados do Google Maps.
"""
import asyncio
import random
import re
        ...
        for card in cards[:limite]:
            try:
                ...
                try:
                    ...
                    # IMPORTANTE: rating = 0.0 para todos invalidos
                    # pode afetar ordenacao e display. Logamos.
                    print(f"[Scraper][WARN] rating invalido: '{rating_txt}'")
                    rating = 0.0
...
def _parsear_resultados(texto: str, limite: int) -> List[Dict]:
    estabelecimentos = []
...
async def _extrair_reviews_de_blocos(page) -> List[Dict]:
    ...
    for block in review_blocks[:25]:
        try:
            ...
            # IMPORTANTE: reviews descartados de blocos principais
            # Perdeu depoimentos de clientes que poderiam ser exibidos
            print(f"[Scraper][WARN] extrair reviews blocos falhou: {e_review}")
            continue
    return depoimentos


async def _extrair_reviews_de_blocos_raw(review_blocks) -> List[Dict]:
            ...
            if texto and len(texto) > 3 and rating_val >= 4:
                if not any(d["texto"] == texto for d in depoimentos):
                    ...
                    if len(depoimentos) >= 10:
                        break
        except Exception as e_review_raw:
            # IMPORTANTE: reviews em destaque descartados
            # Perda de reviews destacados que podem ser mais relevantes
            print(f"[Scraper][WARN] extrair reviews raw falhou: {e_review_raw}")
            continue
    return depoimentos
...
async def _buscar_detalhes(
    ...
    try:
        ...
        if "/maps/place/" not in page.url:
            primeiro = await page.query_selector("a[href*='/maps/place/']")
            if primeiro:
        ...
        if not fast:
            try:
                await page.wait_for_selector("img[src*='googleusercontent']", timeout=3000)
                await asyncio.sleep(0.8)
            except Exception as e_wait:
                # IMPORTANTE: page nao carregou completamente antes do timeout
                # Alguns elementos podem nao estar disponiveis
                print(f"[Scraper][WARN] wait_for_selector imgs falhou: {e_wait}")
        ...
        try:
            ...
            # IMPORTANTE: telefone nao capturado
            # Lead perde informacao de contato importante
            print(f"[Scraper][WARN] captura telefone falhou: {e_tel}")
            pass
        ...
        if not fast:
            try:
                ...
                if btn_horarios:
                    ...
                    except Exception as e_horarios:
                        # IMPORTANTE: horarios nao clicados
                        # Horarios de funcionamento nao serao exibidos
                        print(f"[Scraper][WARN] click horarios falhou: {e_horarios}")
                        pass
                ...
                if not horarios and btn_horarios:
                    lbl = btn_horarios.get_attribute("aria-label") or ""
            ...
            if not endereco_completo:
                ...
                for div in addr_divs[:5]:
                    ...
                    if re.search(r"(Rua|R\.|Av\.|Avenida|Rodovia|Al\.|Alameda|Estr\.|Estrada)", txt) and len(txt) > 10:
                        ...
                        break
        except Exception as e_endereco:
            # IMPORTANTE: endereco nao capturado
            # Informacao de localizacao essencial pode ser perdida
            print(f"[Scraper][WARN] captura endereco falhou: {e_endereco}")
            pass
        ...
        if fast:
            ...
            print(f"[Scraper] {nome}: detalhe rapido, telefone={'sim' if telefone_real else 'nao'}, site={'sim' if website_real else 'nao'}, endereco={endereco_completo[:50]}")
            return {
                "logo": "", "fotos": [], "depoimentos": [],
                "horarios": [], "maps_url": maps_url_final,
        ...
        try:
            ...
            # IMPORTANTE: faixa de preco ausente
            # pode afetar exibicao de precos do estabelecimento
            print(f"[Scraper][WARN] captura faixa_preco falhou: {e_preco}")
            pass
        ...
        try:
            ...
            # IMPORTANTE: modais nao fechados
            # Pode bloquear clicks em reviews subsequentes
            print(f"[Scraper][WARN] fechar modais falhou: {e_modal}")
            pass
        ...
        try:
            ...
            if aba_reviews:
                ...
                except Exception as e_wait_reviews:
                    # IMPORTANTE: reviews da aba nao carregados
                    # Perdeu reviews que estavam na aba de reviews
                    print(f"[Scraper][WARN] wait_for reviews falhou: {e_wait_reviews}")
                    pass
                ...
                for btn in botoes_mais[:10]:
                    ...
                    except Exception as e_btn_mais:
                        # IMPORTANTE: botoes "mais" nao clicados
                        # Reviews expandidos podem nao ser capturados
                        print(f"[Scraper][WARN] click botao mais falhou: {e_btn_mais}")
                        pass
                depoimentos = await _extrair_reviews_de_blocos(page)
                if depoimentos:
        ...
        if not depoimentos:
            try:
                ...
                overview_btn = await page.query_selector("[data-tab-index='0'], button[aria-label*='Visão sobre'], button[aria-label*='Geral']")
                if overview_btn:
                    try:
                        await overview_btn.click(force=True, timeout=5000)
                        await asyncio.sleep(1.5)
                    except Exception as e_overview:
                        # IMPORTANTE: navegacao overview falhada
                        # Pode perder reviews em destaque da visao geral
                        print(f"[Scraper][WARN] click overview falhou: {e_overview}")
                        pass
                destaque_sels = [
                    "div.GHT2ce",
                    "div[data-review-id]",
                ...
                for sel in destaque_sels:
                    ...
                    if review_blocks:
                        break
                if review_blocks:
                    depoimentos = await _extrair_reviews_de_blocos_raw(review_blocks)
                    if depoimentos:
                    ...
                    try:
                        ...
                        if texto_candidatos:
                            ...
                            if not any(d["texto"] == texto for d in depoimentos):
                                ...
                                if len(depoimentos) >= 10:
                                    break
                    except Exception as e_heuristica:
                        # IMPORTANTE: reviews heuristica descartados
                        # Pode perder reviews capturados por heuristica
                        print(f"[Scraper][WARN] heuristica review falhou: {e_heuristica}")
                        continue
                if depoimentos:
