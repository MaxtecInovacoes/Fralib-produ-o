...
def inicializar_database():
    ...
    with engine.connect() as conn:
        ...
        # Traces completos por pipeline run (cada run = trace, cada fase = span).
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS pipeline_traces (
                trace_id VARCHAR(12) PRIMARY KEY,
                ...
                custo_total_usd DECIMAL(10,4) DEFAULT 0,
        ...
        # Log estruturado de erro por step da pipeline.
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS pipeline_error_log (
                ...
                message TEXT,
                traceback_text TEXT,
                categoria VARCHAR(30) DEFAULT 'UNKNOWN',
                payload JSONB DEFAULT '{}'::jsonb,
                criado_em TIMESTAMP DEFAULT NOW()
            )
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_error_log_lead
            ON pipeline_error_log (lead_id, criado_em DESC)
        """))

        # Snapshots de rate-limit da API Anthropic (historico para dashboards).
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS api_usage_snapshots (
                ...
                input_reset VARCHAR(80),
                output_limit INTEGER,
                output_remaining INTEGER,
                output_reset VARCHAR(80),
                req_limit INTEGER,
                req_remaining INTEGER,
                req_reset VARCHAR(80),
                pipelines_available INTEGER DEFAULT 0
            )
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_api_snapshots_time
            ON api_usage_snapshots (captured_at DESC)
        """))

        # Quality Gate results — cada run do Vision QA e registrada aqui.
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS quality_gate_results (
                ...
                vision_issues JSONB DEFAULT '[]'::jsonb,
                vision_strengths JSONB DEFAULT '[]'::jsonb,
                repair_attempted BOOLEAN DEFAULT FALSE,
                repair_success BOOLEAN DEFAULT FALSE,
                repair_fixes JSONB DEFAULT '[]'::jsonb,
        ...
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_qg_lead
            ON quality_gate_results (lead_id, criado_em DESC)
        """))

        # Tabela de tracking de uso de LLM (llm_direct.py salva aqui).
        conn.execute(text("""
        ...
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_llm_usage_modelo
            ON llm_usage (modelo, criado_em DESC)
        """))

        # Tabela de alertas de provider keys (ia_manager + alerting central).
        conn.execute(text("""
        ...
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_provider_alerts_tipo_lido
            ON provider_alerts (tipo, lido, criado_em DESC)
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_provider_alerts_nao_lidos
            ON provider_alerts (criado_em DESC)
            WHERE NOT lido
        """))

        # Tabela de alertas de sistema (pipeline, QA, worker, deploy) — centralizada.
        conn.execute(text("""
        ...
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_system_alerts_categoria
            ON system_alerts (categoria, severity, criado_em DESC)
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_system_alerts_nao_resolvidos
            ON system_alerts (severity, criado_em DESC)
            WHERE NOT resolved
        """))

        # Tabelas RAG (pgvector) — extensão pré-requisito + embeddings de leads/failures.
        # pgvector precisa de CREATE EXTENSION. Se falhar (permissão), não bloqueia bootstrap.
        try:
            conn.execute(text("CREATE EXTENSION IF NOT EXISTS vector"))
        except Exception as ext_err:
            print(f"[Database] Aviso: pgvector nao disponivel: {ext_err}")
        ...
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_lead_embeddings_tenant
            ON lead_embeddings (tenant_id)
        """))
        ...
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS idx_failure_embeddings_step
            ON failure_embeddings (step_name)
        """))
