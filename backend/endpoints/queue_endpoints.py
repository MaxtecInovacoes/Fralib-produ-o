...
def _tenant_scope(usuario: dict):
...
async def queue_status(usuario: dict = Depends(get_current_user)):
    ...
    scope_sql, params = _tenant_scope(usuario)
    with engine.connect() as conn:
        row = conn.execute(text(f"""
            SELECT
                ...
                COUNT(*) as total
            FROM jobs
            WHERE criado_em > NOW() - INTERVAL '24 hours' {scope_sql}
        """), params).fetchone()
    if not row:
    ...
    with engine.connect() as conn:
        row = conn.execute(text(f"""
            SELECT id, tipo, status, tenant_id, attempts, max_attempts,
                   last_phase, last_error, criado_em, iniciado_em, concluido_em,
                   worker_id, priority
            FROM jobs WHERE id = :id {scope_sql}
        """), params).fetchone()
    if not row:
...
async def queue_failed(dias: int = Query(default=7, ge=1, le=30), usuario: dict = Depends(get_current_user)):
    ...
    tenant_id = usuario.get("tenant_id", usuario["id"])
    role = (usuario.get("role") or "").lower()
    scope_sql = "" if role in ("admin", "superadmin") else "AND tenant_id = :tenant_id"
    params = {"dias": dias}
    if scope_sql:
...
async def queue_metrics(dias: int = Query(default=7, ge=1, le=30), usuario: dict = Depends(get_current_user)):
    ...
    with engine.connect() as conn:
        ...
        franz_row = conn.execute(text(f"""
            ...
            FROM jobs
            WHERE criado_em > NOW() - make_interval(days => :dias)
              AND tipo IN ('franz_outreach', 'bryan_outreach')
              {scope_sql}
        """), params).fetchone()
    ...
    return {"pipeline": pipeline, "franz": franz, "bryan": franz}
