...
def _site_slug_from_url_or_name(site_url: str, lead_nome: str) -> str:
    if site_url and "/sites/" in site_url:
        slug_url = site_url.rstrip("/").split("/sites/")[-1].rstrip("/")
        slug = slug_url.split("/")[-1] if "/" in slug_url else slug_url
    else:
        slug = ""
    if not slug:
        slug = _re.sub(r"[^a-z0-9]+", "-", (lead_nome or "").lower()).strip("-")[:50]
    if not _re.fullmatch(r"[a-z0-9][a-z0-9-]{0,80}", slug or ""):
...
async def editar_secao_endpoint(
    ...
    from agents.section_editor import editar_secao, listar_secoes
    ...
    if req.secao not in secoes_disponiveis and req.secao != "geral":
        raise HTTPException(
            status_code=400,
            detail=f"Secao nao encontrada. Disponiveis: {secoes_disponiveis}",
        )
    ...
    return {
        ...
        "lead_id": req.lead_id,
        "html_path": html_path,
        "tamanho_html": len(html_editado),
        "mensagem": f"Secao {req.secao} editada com sucesso",
    }
...
async def listar_secoes_endpoint(
    ...
    from agents.section_editor import listar_secoes
