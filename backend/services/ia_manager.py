...
def _env_fallback(provider: str) -> tuple[str, str] | None:
    """Read fallback credentials at call time so rotation does not require reload."""
    config = _ENV_FALLBACK_CONFIG.get(provider)
    if not config:
        return None
    key_name, url_name, default_url = config
    return os.getenv(key_name, ''), os.getenv(url_name, default_url)
...
def pick_key(provider: str, *, _attempt: int = 0) -> Optional[tuple[str, str, Optional[int]]]:
    ...
    """
    if _attempt >= 20:
        env = _env_fallback(provider)
        return (env[0], env[1], None) if env and env[0] else None
    ...
    if not plain:
        ...
        return pick_key(provider, _attempt=_attempt + 1)

    if not base_url:
        base_url = _default_base_url(provider)
    if base_url and not is_allowed_llm_url(base_url):
        mark_failure(key_id, 'base_url_not_allowed', cooldown_seconds=3600)
        return pick_key(provider, _attempt=_attempt + 1)
