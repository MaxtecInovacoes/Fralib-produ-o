...
from validation_enforcer import require_rag, require_guidelines  # ✅ Validação obrigatória
...
class AnimationSpec(BaseModel):
    ...
    def from_any(cls, data) -> "AnimationSpec":
        ...
        if isinstance(data, dict):
            return cls(
                name=str(data.get("name", data.get("id", data.get("animation", "fade-in")))),
                type=str(data.get("type", data.get("animation_type", data.get("effect", "fade-in")))),
                target=str(data.get("target", data.get("element", data.get("selector", "section")))),
...
class SectionSpec(BaseModel):
    ...
    def normalize_schema_org_field(cls, v):
        ...
        if isinstance(v, dict): return v.get("type", v.get("name", str(v)))
        return str(v)
...
class DesignerPRD(BaseModel):
        ...
        if isinstance(v, dict):
            ...
            for key, val in v.items():
                if isinstance(val, dict):
                    ...
                    result.append({"name": key.capitalize(), "required": True, "components": ["cta"], "data_source": val})
            return result if result else [{"name": "Hero", "required": True, "components": ["hero-cta"], "data_source": "Fallback"}]
        if not isinstance(v, list):
            return [{"name": "Hero", "required": True, "components": ["hero-cta"], "data_source": "Fallback"}]
    ...
    def normalize_animations(cls, v):
        ...
        if not isinstance(v, list):
            return [{"name": "fade-in", "type": "fade-in", "target": "section", "trigger": "scroll"}]
        result = []
        for item in v:
            if isinstance(item, dict):
                result.append({
                    "name": str(item.get("name", item.get("id", item.get("animation", "fade-in")))),
                    "type": str(item.get("type", item.get("animation_type", item.get("effect", item.get("kind", "fade-in"))))),
                    "target": str(item.get("target", item.get("element", item.get("selector", item.get("applies_to", "section"))))),
                    "trigger": str(item.get("trigger", item.get("event", item.get("on", "scroll")))),
                    "duration": str(item.get("duration", item.get("timing", "0.6s"))),
                    "easing": str(item.get("easing", item.get("ease", "ease-out"))),
                })
            elif isinstance(item, str):
                result.append({"name": item, "type": "fade-in", "target": "section", "trigger": "scroll"})
        if not result:
            result = [{"name": "fade-in", "type": "fade-in", "target": "section", "trigger": "scroll"}]
        return result
    business_name: str
    ...
    def normalize_photos(cls, v):
        ...
        for item in v:
            ...
            elif isinstance(item, dict): result.append(str(item.get("url", item.get("src", ""))))
    ...
    def normalize_components_21dev(cls, v):
        if not isinstance(v, list): return ["hero-cta"]
        result = []
        for item in v:
            if isinstance(item, str): result.append(item)
            elif isinstance(item, dict): result.append(str(item.get("id", item.get("name", item.get("component", str(item))))))
            else: result.append(str(item))
    ...
    def normalize_jina_insights(cls, v):
        if not v: return ""
        if isinstance(v, str): return v
        if isinstance(v, dict): return v.get("summary", v.get("insights", v.get("content", str(v))))
        return str(v)
    ...
    def normalize_competitor_analysis(cls, v):
        if not v: return ""
        if isinstance(v, str): return v
        if isinstance(v, dict): return str(v.get("summary", str(v)))
        return str(v)
    ...
    def normalize_anti_patterns(cls, v):
        ...
        for item in v:
            if isinstance(item, str): result.append(item)
            elif isinstance(item, dict): result.append(str(item.get("pattern", item.get("name", item.get("description", str(item))))))
            else: result.append(str(item))
        return result
    ...
    def normalize_schema_org(cls, v):
        if not isinstance(v, list): return ["LocalBusiness"]
        result = []
        for item in v:
            if isinstance(item, str): result.append(item)
    ...
    def fill_missing_fields(self) -> 'DesignerPRD':
        ...
        if not self.sections:
            self.sections = [
                SectionSpec(name="Hero", required=True, components=["hero-cta"], data_source="Hunter V2"),
                SectionSpec(name="Sobre", required=True, components=["about-text", "about-image"], data_source="Hunter V2"),
                SectionSpec(name="Servicos", required=True, components=["services-grid"], data_source="Hunter V2"),
                SectionSpec(name="Depoimentos", required=False, components=["reviews-list"], data_source="Hunter V2"),
                SectionSpec(name="Planos", required=True, components=["pricing-cards"], data_source="Hunter V2"),
                SectionSpec(name="Localizacao", required=True, components=["map-embed", "address-info"], data_source="Hunter V2"),
                SectionSpec(name="Contato", required=True, components=["contact-form", "whatsapp-cta"], data_source="Hunter V2"),
            ]
...
DESIGNER_INSTRUCTIONS = '''Você é o Designer PRD, especialista em criar PRDs detalhados para sites.

**TAREFA:**
Gere um PRD completo em JSON com:

1. **Wireframe textual** (sections)
2. **Paleta de cores única** (anti-padrão dos concorrentes)
3. **5 animações específicas** (parallax, fade-in, hover, scroll-reveal, pulse)
4. **Dados validados** (reviews_count exato, não "2+")
5. **Google Maps embed URL**
6. **Componentes 21dev necessários**
...
**FORMATO DE SAÍDA JSON:**
{
  ...
  "reviews_count": 120,
  "reviews_rating": 4.8,
  "reviews_list": [{"autor": "Nome", "texto": "Review", "rating": 5}],
  ...
  "competitor_analysis": "Análise dos concorrentes",
  "anti_patterns": ["Não usar paleta X", "Evitar layout Y"],
  "schema_org_types": ["LocalBusiness", "Review"]
}

**CRÍTICO:**
- reviews_count DEVE ser número exato (não "2+")
- Paleta DEVE ser única (não repetir concorrentes)
- 5 animações DEVEM ser específicas
...
- NUNCA usar paleta generica azul (#3b82f6, #2563eb) - usar paleta do briefing
- SEMPRE incluir secao LGPD (banner de cookies) como secao obrigatoria
- SEMPRE incluir Schema.org LocalBusiness completo
- SEMPRE respeitar dark/light mode indicado no briefing do Theo
- Se briefing indicar DARK MODE: background deve ser #0a0a0a ou #111111
- Se briefing indicar LIGHT MODE: background deve ser #ffffff ou #f9fafb
'''
...
def gerar_prd(
    ...
    """
    Gera PRD detalhado para o Liam implementar
    """
...
- Respeitar MODO VISUAL do briefing do Theo (DARK ou LIGHT)
- Incluir secao LGPD como secao obrigatoria no PRD
...
"""
    ...
    PRD_SCHEMA = {
        "type": "object",
        "required": ["sections", "color_palette", "typography", "animations",
                     "business_name", "reviews_count", "reviews_rating", "reviews_list",
                     "address", "phone", "photos", "google_maps_embed",
                     "components_21dev", "competitor_analysis", "anti_patterns", "schema_org_types"],
        "properties": {
            "sections": {"type": "array", "items": {"type": "object", "required": ["name","required","components","data_source"], "properties": {"name": {"type": "string"}, "required": {"type": "boolean"}, "components": {"type": "array", "items": {"type": "string"}}, "data_source": {"type": "string"}, "schema_org": {"type": "string"}}}},
            "color_palette": {"type": "object", "required": ["primary","secondary","accent","background","text","reasoning"], "properties": {"primary": {"type": "string"}, "secondary": {"type": "string"}, "accent": {"type": "string"}, "background": {"type": "string"}, "text": {"type": "string"}, "reasoning": {"type": "string"}}},
            "typography": {"type": "object", "required": ["heading","body","accent"], "properties": {"heading": {"type": "string"}, "body": {"type": "string"}, "accent": {"type": "string"}}},
            "animations": {"type": "array", "items": {"type": "object", "required": ["name","type","target","trigger"], "properties": {"name": {"type": "string"}, "type": {"type": "string"}, "target": {"type": "string"}, "trigger": {"type": "string"}, "duration": {"type": "string"}, "easing": {"type": "string"}}}},
    ...
    try:
        ...
        print(f"[Designer PRD] Structured Output recebido: {list(response_json.keys())[:5]}...")

        prd = DesignerPRD(**response_json)
        prd.cidade = cidade
        prd.segmento = segmento
        print(f"[Designer PRD] PRD gerado: {len(prd.sections)} secoes, {len(prd.animations)} animacoes")
        return prd
        ...
        return DesignerPRD(
            ...
            animations=[
                AnimationSpec(name="hero-fade", type="fade-in", target="hero", trigger="load", duration="0.6s", easing="ease-out")
            ],
