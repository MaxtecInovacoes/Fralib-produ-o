...
RAG_DIR = Path("/root/fralib/backend/agents/rag_knowledge")
...
def buscar_contexto_rag(query: str, agente: str, top_k: int = 5) -> str:
    """Busca contexto RAG — carrega arquivo principal + blocos liam_*.md"""
    try:
        ...
        if content:
            ...
            full_context = "\n\n".join(chunks)
            print(f"[RAG Local] ✅ {agente}: {len(full_context)} chars ({len(chunks)} chunks, {1 + len(extra_files)} arquivos)")
            return full_context
        ...
        if general_file.exists():
            ...
            chunks = chunk_rag_content(content, max_chars=25000)
            full_context = "\n\n".join(chunks)
            print(f"[RAG Local] ⚠️ Contexto geral para {agente}: {len(full_context)} chars")
            return full_context

        print(f"[RAG Local] ⚠️ Nenhum contexto para {agente}")
        return ""
...
def get_agent_temperature(agente: str) -> float:
    ...
    temperatures = {
        ...
        'theo': 0.8, 'liam': 0.9, 'liz': 0.2, 'bryan': 0.4
    }
...
def mark_rag_used(agente: str):
    ...
    print(f"[RAG Local] ✅ RAG marcado como usado para {agente}")
