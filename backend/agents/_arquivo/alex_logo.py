...
def processar_logo(logo_url: Optional[str], assets_dir: str = "/tmp") -> dict:
    ...
    if not logo_url.startswith('http') and _os_logo.path.exists(logo_url):
        # Path local já processado — usar diretamente sem requests.get
        print("[Alex] Logo é path local, usando diretamente")
        logo_sem_fundo = logo_url
    else:
