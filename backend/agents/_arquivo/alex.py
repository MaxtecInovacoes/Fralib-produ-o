...
def processar_imagens(input_data: AlexInput) -> AlexOutput:
    ...
    print("[ALEX] 2/6 Identificando logo via heurística Python...")
    logo_url_identificada = None
    try:
        from PIL import Image as _PILImage
        import io as _io

        def _score_logo(foto_dict):
            """Pontua probabilidade de ser logo: imagem pequena + quadrada = logo."""
            local = foto_dict.get("webp", "") or foto_dict.get("original", "")
            if not local:
                return 0
            try:
                if local.startswith("/var/www/") and os.path.exists(local):
                    img = _PILImage.open(local)
                else:
                    resp = _req.get(local, timeout=5)
                    img = _PILImage.open(_io.BytesIO(resp.content))
                w, h = img.size
                ratio = min(w, h) / max(w, h) if max(w, h) > 0 else 0
                area = w * h
                score = 0
                # Quadrada ou quase quadrada (logo tende a ser quadrado)
                if ratio > 0.85: score += 40
                elif ratio > 0.70: score += 20
                # Pequena (logo tende a ser menor que foto de fachada)
                if area < 50000: score += 30
                elif area < 150000: score += 15
                # Fundo transparente = logo
                if img.mode == "RGBA":
                    score += 30
                return score
            except:
        ...
        scores = [(i, _score_logo(f)) for i, f in enumerate(todas_webp[:9])]
        scores.sort(key=lambda x: x[1], reverse=True)
        best_idx, best_score = scores[0] if scores else (0, 0)

        if best_score >= 50:
            # Alta confiança — usa heurística
            logo_url_identificada = todas_webp[best_idx].get("webp") or todas_webp[best_idx].get("original")
            print(f"[ALEX] Logo identificada por heurística: imagem {best_idx+1} (score={best_score})")
        else:
            # Baixa confiança — fallback para Vision
            print(f"[ALEX] Heurística inconclusiva (score={best_score}), usando Vision...")
            cp = []
            idx_list = []
            for i, f2 in enumerate(todas_webp[:9]):
                ...
                if local and local.startswith("/var/www/") and os.path.exists(local):
                    ...
                    idx_list.append(i)
            if cp:
                cp.append({"type": "text", "text": "Identifique o LOGO. Responda APENAS: LOGO:N"})
                ...
                pl = {"model": "claude-haiku-4-5", "max_tokens": 10, "temperature": 0.0,
                      "system": "Identifica logo. Responda apenas LOGO:N",
                ...
                if lm and int(lm.group(1)) > 0:
                    ...
                    if 0 <= ix < len(idx_list):
                        ...
                        print("[ALEX] Logo identificada via Vision: imagem " + str(ix+1))
    except Exception as e:
        print("[ALEX] Identificação de logo falhou: " + str(e))
        raise
