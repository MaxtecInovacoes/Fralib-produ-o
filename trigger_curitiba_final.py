"""
Trigger: reprocessa 'Curitiba Fitness' diretamente no worker e monitora até conclusão.
Validação final: procura design-tokens, hermetic sections e native FAQ no HTML publicado.
"""
...
# ── 1. Marcar lead como capturado e resetar flags ─────────────────────
with engine.begin() as conn:
    row = conn.execute(
        text("SELECT nome, segmento, cidade, status, url_site FROM leads WHERE id=:i"),
        {"i": LEAD_ID},
    ).fetchone()
    if not row:
...
# ── 2. Enfileirar job via job_queue ────────────────────────────────────
...
while time.time() - t0 < MAX_WAIT:
    ...
    with engine.connect() as conn:
        s = conn.execute(
            text("SELECT status FROM leads WHERE id=:i"),
            {"i": LEAD_ID},
        ).fetchone()
...
# ── 5. Validar HTML publicado ───────────────────────────────────────────
site_url = fd.get("url_site") or ""
if site_url:
    # O builder publica em /var/www/fralib/sites/<tenant>/<lead>/
    # Tenta localizar o arquivo index.html pelo site_url ou pelo diretório esperado
    slug = re.sub(r'[^a-z0-9-]+', '-', LEAD_NOME.lower()).strip('-')
    possible_paths = [
        f"/var/www/fralib/sites/{TENANT_ID}/{slug}/index.html",
        f"/var/www/fralib/sites/{TENANT_ID}/{LEAD_ID}/index.html",
        f"/var/www/fralib/sites/{slug}/index.html",
    ]
    # Fallback: listar o diretório do tenant
    ...
    if found_path:
        ...
        checks = {
            ...
            "unprefixed :root vars": ':root{--bg:' in html or ':root {' in html.lower() or '--bg:' in html,
        ...
        # Amostra do bloco design-tokens
        idx = html.find('design-tokens')
        if idx != -1:
            print(f"Tokens snippet: {html[max(0,idx-10):idx+200]!r}")
    else:
