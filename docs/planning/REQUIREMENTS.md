...
- [ ] **BRYAN-01**: State machine avança corretamente: intro → proof → link → value → price → close
- [ ] **BRYAN-02**: Cada estado tem mensagem personalizada por tier (PREMIUM=consultivo, BASIC=hard sell)
- [ ] **BRYAN-03**: Transição automática de estado baseada na resposta do lead
- [ ] **BRYAN-04**: Timeout por estado (lead sem resposta em X horas → próximo estado ou encerrar)
- [ ] **BRYAN-05**: Log de cada transição de estado no PostgreSQL
...
- [ ] **STRIPE-04**: Bryan envia link de pagamento Stripe no estado "price" da state machine
- [ ] **STRIPE-05**: Landing page exibe planos com preços e botão de checkout Stripe

### Pipeline (Correções e Melhorias)

- [ ] **PIPE-01**: Reprocessar pipeline completo funciona (Liam → Liz → Deploy → Bryan no reprocessar)
...
| BRYAN-01 | Phase 2 | Pending |
| BRYAN-02 | Phase 2 | Pending |
| BRYAN-03 | Phase 2 | Pending |
| BRYAN-04 | Phase 2 | Pending |
| BRYAN-05 | Phase 2 | Pending |
