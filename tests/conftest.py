...
def _assert_safe_test_database_url(value: str) -> None:
    ...
    if "test" not in db_name or not safe_host:
        raise RuntimeError(f"Unsafe test database URL blocked: {value}")
...
def test_database_url():
    ...
    return os.getenv(
        "TEST_DATABASE_URL",
        "postgresql://postgres@localhost:5433/fralib_test"
    )
...
def test_user(db_session, test_user_data):
    ...
    query = text("""
        INSERT INTO users (
            email, password_hash, nome, tenant_id,
            plano, plan, status, creditos, creditos_max, email_confirmado
        )
        VALUES (
            :email, :password_hash, :nome, :tenant_id,
            'trial', 'free', 'trial', 1, 1, true
        )
