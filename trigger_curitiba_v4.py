"""Check state and enqueue Curitiba Fitness reprocess — coluna criado_em."""
import sys
...
with engine.connect() as conn:
    ...
    print("PIPELINE_STATE:", dict(state._mapping) if state else "NO STATE (first run)")

    # List active jobs with correct column name
    jobs = conn.execute(
        text("SELECT id, tipo, status, payload, criado_em FROM jobs WHERE status IN ('pending','running','queued') ORDER BY criado_em DESC LIMIT 10")
    ).fetchall()
    print("=== ACTIVE JOBS ===")
    for j in jobs or []:
        print(dict(j._mapping))
    if not jobs:
        print("NO ACTIVE JOBS — WORKER IS FREE")
