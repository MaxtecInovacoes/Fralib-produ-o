...
"""
import os, sys, requests, psycopg2, re, json
...
def get_db():
    ...
    return psycopg2.connect(host=host, port=int(port), user=user, password=pwd or '', dbname=dbname)
...
def save_to_db(data):
    ...
    try:
        ...
        """, (
            data['input_limit'], data['input_remaining'], data['input_reset'],
            data['output_limit'], data['output_remaining'], data['output_reset'],
            data['req_limit'], data['req_remaining'], data['req_reset']
        ))
        conn.commit()
        cur.close()
        conn.close()
...
def dashboard(data):
    inp_used = round((1 - data['input_remaining'] / data['input_limit']) * 100, 1) if data['input_limit'] else 0
    out_used = round((1 - data['output_remaining'] / data['output_limit']) * 100, 1) if data['output_limit'] else 0
    req_used = round((1 - data['req_remaining'] / data['req_limit']) * 100, 1) if data['req_limit'] else 0
    ...
    reset_time = data['input_reset'][:19].replace('T', ' ') if data['input_reset'] else '?'
